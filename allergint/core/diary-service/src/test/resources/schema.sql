...
DROP TABLE IF EXISTS allergy_trigger;
...
(
    ...
    timestamp         TIMESTAMP WITH TIME ZONE NOT NULL,
    CONSTRAINT fk_weather_diary FOREIGN KEY (diary_page_id) REFERENCES diary (id) ON DELETE CASCADE,
    PRIMARY KEY (id)
);
...
(
    ...
    medicine_description VARCHAR(1000),
    medicine_type        VARCHAR(50),
    dose                 DOUBLE PRECISION NOT NULL,
    diary_page_id        UUID             NOT NULL,
    dose_measure_type    VARCHAR(50),
    PRIMARY KEY (id),
    CONSTRAINT fk_medicine_diary FOREIGN KEY (diary_page_id) REFERENCES diary (id) ON DELETE CASCADE
);
...
(
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_symptom_diary FOREIGN KEY (diary_page_id) REFERENCES diary (id) ON DELETE CASCADE
);

CREATE TABLE allergy_trigger
(
    id            UUID DEFAULT gen_random_uuid(),
