  ...
  return (
    <div className="card">
      ...
      <div className={style.severity}>
        ...
        <div>
          Частота симптомов: <span className={style.span}>{chastota}</span>
        </div>
      </div>
        ...
        {al.map((val, index) => {
          return (
            <li className={style.listItem} key={index}>
              ...
              {val.level === 1 && <div className={style.weak}>слабо</div>}
              {val.level === 2 && <div className={style.medium}>средне</div>}
              {val.level === 3 && <div className={style.strong}>сильно</div>}
            </li>
          )
        })}
      </ul>
