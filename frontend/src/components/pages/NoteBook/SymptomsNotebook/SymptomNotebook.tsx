...
function SymptomNotebook() {
  const [activeButton, setActiveButton] = useState<SymptomStatus>(null)
  ...
  const handleStatusClick = (status: SymptomStatus) => {
    const now = new Date()
    setActiveButton(status)
    setClickTime(now)
  }

  const getButtonClassName = (status: SymptomStatus) => {
    return `${styles.symptomListItemStatusBtn} ${
      activeButton === status ? styles.symptomListItemStatusBtnActive : ''
    }`
  }
  ...
  return (
    <div className="card">
      ...
      <div className={styles.symptomList}>
        <div className={styles.symptomListItem}>
          ...
          <div className={styles.symptomListItemStatus}>
            {statusButtons.map((button) => (
              <button
                ...
                onClick={() => handleStatusClick(button.id)}
              >
                {button.label}
              </button>
            ))}
          ...
          <div className={styles.symptomListItemTime}>
            <p>
              Время: <span>{clickTime ? formatTime(clickTime) : '--:--'}</span>
            </p>
          </div>
        </div>
      </div>
